...
namespace Hatt.Infrastructure.Persistence;
    ...
    : DbContext(options)
{
    ...
    public DbSet<UserProgressSnapshot> UserProgressSnapshots => Set<UserProgressSnapshot>();
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<UserDevice>(e =>
        {
            ...
            // KVKK: deleting a user removes every trace.
            e.HasOne(d => d.User)
        ...
        b.Entity<LeagueMember>(e =>
        {
            ...
            e.HasOne(m => m.Cohort)
                ...
                .OnDelete(DeleteBehavior.Cascade);
            // KVKK: account deletion removes league rows too.
            e.HasOne(m => m.User)
                .WithMany()
        ...
        {
            e.Property(i => i.Provider).HasConversion<string>().HasMaxLength(16);
            e.Property(i => i.Subject).HasMaxLength(256);
            e.HasIndex(i => new { i.Provider, i.Subject }).IsUnique();
            e.HasOne(i => i.User)
        ...
        b.Entity<UserProgressSnapshot>(e =>
        {
            e.HasKey(s => s.UserId);
            e.Property(s => s.DataJson).HasColumnType("jsonb");
            e.HasOne(s => s.User)
                .WithOne(u => u.ProgressSnapshot)
                .HasForeignKey<UserProgressSnapshot>(s => s.UserId)
                .OnDelete(DeleteBehavior.Cascade);
        });

        b.Entity<TelemetryEvent>(e =>
        {
            e.Property(t => t.EventName).HasMaxLength(64);
            e.Property(t => t.PropertiesJson).HasColumnType("jsonb");
            e.HasIndex(t => t.EventName);
